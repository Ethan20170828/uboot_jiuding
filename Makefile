...
# 现在是刚开始进入根目录，所以SRCTREE和TOPDIR是一样的
...
export MKCONFIG
# 如果$(OBJTREE)和$(SRCTREE)不相等的情况下，执行REMOTE_BUILD	:= 1
ifneq ($(OBJTREE),$(SRCTREE))
REMOTE_BUILD	:= 1
export REMOTE_BUILD
endif
# OBJTREE和SRCTREE不相等，那么obj := $(OBJTREE)；如果相等，则obj为空
...
# load other configuration，config.mk文件主要做两件事，一个是链接脚本，另一个是引用自动生成的autoconf.mk文件，在Makefile中指导整个编译过程

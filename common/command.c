 ...
 */
// 传参: argc表示传参的个数；argv表示每个传参参数的内容
 ...
 */
// find_cmd函数的任务是从当前uboot的命令集中查找是否有某个命令。
// 如果找到则返回这个命令结构体的指针，如果未找到返回NULL。
// 接收的参数就是传过来的命令
// 返回的是命令所对应的结构体
// 函数的实现思路很简单，如果不考虑命令带点的情况(md.b md.w这种)就更简单了。
// 查找命令的思路其实就是for循环遍历数组的思路，不同的是数组的起始地址和结束地址是用地址值来给定的，
// 数组中的元素是结构体变量类型。
cmd_tbl_t *find_cmd (const char *cmd)
{
	cmd_tbl_t *cmdtp;
// __u_boot_cmd_start是在链接脚本中定义的；也可以理解为cmdtp_temp结构体指针指向了命令集的首地址 
	 ...
	 */
	// strchr函数就是判断cmd(cmd就是查找到的命令)中是否有.
	// 如果cmd中没有.则strchr函数的返回值是NULL，则len=strlen (cmd)；否则len=(p - cmd)，也就是计算点前面的大小
	len = ((p = strchr(cmd, '.')) == NULL) ? strlen (cmd) : (p - cmd);
	// for循环的起始地址自定义链接地址的起始地址，终止条件时自定义链接地址的结束地址，地址++(指针++，指针的数据类型是结构体指针，每次++往后走的是一个结构体元素的大小)
	for (cmdtp = &__u_boot_cmd_start;
	     cmdtp != &__u_boot_cmd_end;
	     cmdtp++) {	// cmdtp是结构体指针，cmdtp++是走一个结构体变量，而不是+4或者+8
	    // 将用户自定义链接地址中的所有命令cmdtp->name与我们要找的cmd命令进行对比，
	    // 如果相等，则这个命令就是我们要找的，然后在看len
	    // 如果len的长度相等，那么这个命令就是我们要找的命令了
	    // 总结: 这两个if判断语句，第一个if是判断命令名字是否相等，第二个if是判断命令的长度是否相等
		if (strncmp (cmd, cmdtp->name, len) == 0) {
			if (len == strlen (cmdtp->name))
				return cmdtp;	/* full match */
			// 下面这两行是处理len不相等的情况，譬如(md/md.b/md.w/md.l等)

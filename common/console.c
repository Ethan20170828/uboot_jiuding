...
void puts (const char *s)
{
	...
	if (gd->flags & GD_FLG_SILENT)
		return;
#endif
	// 如果gd->flags中有GD_FLG_DEVINIT标志位，调用fputs输出，如果没有则调用serial_puts输出
	if (gd->flags & GD_FLG_DEVINIT) {
		/* Send to the standard output */
		fputs (stdout, s);
	} else {
...
// console(控制台)的第一阶段初始化。f表示是第一阶段初始化，r表示第二阶段初始化。
// 有时候初始化函数不能一次一起完成，中间必须要夹在一些代码，因此将完整的一个模块的初始化分为两个阶段。
// console_init_r在start_armboot函数中进行初始化
// 放在这个目录下，就是对软件层次的初始化
// 现在控制台还是不可以用的
...
/* Called after the relocation - use desired console functions */
// 这个函数就是console的纯软件架构方面的初始化(说白了就是去给console相关的数据结构中填充相应的值)，
// 所以属于纯软件配置类型的初始化。
// uboot的console实际上并没有干有意义的转化，它就是直接调用的串口通信的函数。
// 所以用不用console实际并没有什么分别。(在Linux内核内console就可以提供缓冲机制等不用console不能
// 实现的东西)。
int console_init_r (void)
{

...
// movi_write_env真正保存环境变量的函数(写SD卡)，
// 将ddr中的环境变量数组(default_environment这个数组，大小16kb，刚好32个扇区)写入iNand中的ENV分区中
...
// 没有一种芯片叫auto的，env_auto.c中使用宏定义的方式去条件编译了各种常见的Flash芯片
// 如movinand、norflash、nand等。然后在程序中读取INF_REG(OMpin年内部对应的寄存器)从而知道
// 我们的启动介质，然后调用这种启动介质对应的操作函数来操作。
int saveenv(void)
{
	// 我们的是CONFIG_S5PC110
#if defined(CONFIG_S5PC100) || defined(CONFIG_S5PC110) || defined(CONFIG_S5P6442)
// INF_REG3_REG(OMpin内部对应的寄存器)从而知道我们的启动介质，然后调用这种启动介质对应的操作函数来操作。
	if (INF_REG3_REG == 2)
		saveenv_nand();
	else if (INF_REG3_REG == 3)	// start.S中定义为3
		saveenv_movinand();		// 这里我们走的是这个函数	iNand和movinand是一个东西，三星就movinand，SanDisk叫iNand

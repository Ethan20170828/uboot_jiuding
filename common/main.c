...
/****************************************************************************/
// main_loop函数做的都是与具体平台无关的工作，主要包括初始化启动次数限制机制，设置软件版本号，打印启动信息，解析命令等
void main_loop (void)
{
	// 解析器
...
int run_command (const char *cmd, int flag)
{
...
#endif
	// 按键盘的ctrl + c 来终止程序
	// clear_ctrlc是为了把ctrl + c 功能去除掉
	clear_ctrlc();		/* forget any previous Control C */

	// cmd本身或者cmd指向的位置是空的，返回都是-1
	if (!cmd || !*cmd) {
	...
	// 1、控制台命令获取
	// 把cmd(这个cmd是run_command函数的传参，指的是传过来的命令)复制到cmdbuf中
	strcpy (cmdbuf, cmd);
...
#endif
// 这个while循环是对cmdbuf(这个里面的命令是刚才从cmd中复制过来的)中的命令进行解析
	while (*str) {
		...
		// 3、命令集中查找命令
		// 关键点在于find_cmd函数如何查找到这个命令是不是uboot的合法支持的命令? 
		// 这取决于uboot的命令体系机制(uboot是如何完成命令的这一套设计的，命令如何去注册、存储、管理、索引)
		// find_cmd函数去uboot的命令函数集合当中搜索有没有argv[0])命令
		// cmdtp指的是我们找到的那个命令在命令集的哪个位置存放
		if ((cmdtp = find_cmd(argv[0])) == NULL) {
			// 在uboot命令行输入一个非法的命令时打印的信息在这里
		...
		// 如果参数个数大于该命令可接收的最多参数，那么打印出该命令的简短使用说明

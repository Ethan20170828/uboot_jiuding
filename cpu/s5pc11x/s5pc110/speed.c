...
/* return ARMCORE frequency ；CPU使用的时钟1GHz*/
...
ulong get_HCLK(void)
{
	...
	div = CLK_DIV0_REG;
	// apll_ratio = 0	；ARMCLK = MOUT_MSYS / 1，(div>>0) = 0
	apll_ratio = ((div>>0) & 0x7);
	// hclk_msys_ratio = 4	；HCLK_MSYS = ARMCLK / 5，(div>>8) = 4 = 0x100
	...
	default:
		fclk = get_FCLK();		// 走的是这条路
		break;
	}
	// fclk/((apll_ratio+1)*(hclk_msys_ratio+1)) = fclk/5
	return fclk/((apll_ratio+1)*(hclk_msys_ratio+1));
}

/* return PCLK frequency 函数的返回值是PCLK的频率=66MB*/
ulong get_PCLK(void)
{
	ulong hclk;
	uint div = CLK_DIV0_REG;
	// PCLK_MSYS = HCLK_MSYS / 2 可以推出(div>>12)=0x1
	// 0x1 & 0x7 = 1
	uint pclk_msys_ratio = ((div>>12) & 0x7);

	hclk = get_HCLK();	
	// hclk/(pclk_msys_ratio+1) = hclk/2
...
int print_cpuinfo(void)
{
	...
	// get_FCLK()返回值为1000000000；get_HCLK()返回值为200000000；get_PCLK()返回值为100000000

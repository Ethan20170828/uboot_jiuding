...
/* skip to load BL2 */
//#define FAST_BOOT		1
// ÏÂÃæ¶¨ÒåµÄÕâÐ©Öµ¶¼ÊÇÍ¨¹ýstart.SÖÐÓ²¼þ¶¨ÒåµÄÖµµÃµ½µÄ
#define MEMORY_BASE_ADDRESS		0x30000000			// 30000000-3fffffff  Õâ¸öµØÖ·ÊÇµÚÒ»¿éÄÚ´æËùÔÚµÄÎïÀíµØÖ·µÄÆðÊ¼µØÖ·
//#define MEMORY_BASE_ADDRESS	0x20000000			// ÈýÐÇÒÔÇ°µÄ£¬±»×¢ÊÍµôÁË£¬¿ÉÒÔÉ¾³ý
#define MEMORY_BASE_ADDRESS2	0x40000000			// 40000000-4fffffff£¬Õâ¸öµØÖ·ÊÇµÚ¶þ¿éÄÚ´æËùÔÚµÄÎïÀíµØÖ·µÄÆðÊ¼µØÖ·
...
// ÕâÈý¸öºêÓÃÀ´¾ö¶¨ubootÊÇ·ñ¸økernel´«ÕâÐ©²ÎÊý
#define CONFIG_SETUP_MEMORY_TAGS	// tag_mem£¬´«²ÎÄÚÈÝÊÇÄÚ´æÅäÖÃÐÅÏ¢
#define CONFIG_CMDLINE_TAG			// tag_cmdline£¬´«²ÎÄÚÈÝÊÇÆô¶¯ÃüÁîÐÐ²ÎÊý£¬Ò²¾ÍÊÇuboot»·¾³±äÁ¿µÄbootargs
#define CONFIG_INITRD_TAG			// tag_initrd£¬
...
#endif
// Ê±ÖÓµÄ´úÂëÅäÖÃÖµ¶¼ÔÚÕâ¸öÎÄ¼þÖÐ£¬Èç¹ûÒÆÖ²Ê±ÐèÒª¸ü¸ÄCPUµÄÊ±ÖÓÅäÖÃ£¬¸ù±¾²»ÐèÒª¶¯´úÂë
// Ö»ÐèÒªÔÚÕâ¸öÎÄ¼þÖÐ¸ü¸ÄÅäÖÃÖµ¼´¿É¡£
...
// ÕâÐ©A¡¢M¡¢E¡¢VµÄÅäÖÃ¶¼ÊÇÍ¨¹ý²éÊý¾ÝÊÖ²áÖÐµäÐÍÊ±ÖÓÅäÖÃÖµµÄÍÆ¼öÅäÖÃµÃÀ´µÄ¡£ÕâÐ©ÅäÖÃÖµÊÇÈýÐÇÍÆ¼öµÄ£¬Òò´Ë¹¤×÷×îÎÈ¶¨
...
#if defined(CONFIG_MCP_SINGLE)
// ÔÚÂã»úÖÐDMC0µÄ256MBÄÚ´æµØÖ··¶Î§Îª0x2000000-0x2FFFFFFF
// ÔÚubootÖÐDMC0µÄ256MBÄÚ´æµØÖ··¶Î§Îª0x3000000-0x3FFFFFFF
// ÔÚubootÖÐ£¬¿ÉÓÃµÄÎïÀíµØÖ··¶Î§Îª: 0x30000000-4FFFFFFF£¬Ò»¹²512MB£¬ÆäÖÐ30000000-3FFFFFFFÎªDMC0£¬40000000-4FFFFFFFÎªDMC1
...
#endif
// 505ÐÐ-511ÐÐÊ¹ÓÃ±ê×¼µÄºê¶¨ÒåÀ´ÅäÖÃDDRÏà¹ØµÄ²ÎÊý
// Ö÷ÒªÅäÖÃÁËÕâÃ´¼¸Æ¬ÐÅÏ¢: ÓÐ¼¸Æ¬DDRÄÚ´æ¡¢Ã¿Ò»Æ¬DDRµÄÆðÊ¼µØÖ·¡¢³¤¶È¡£
// ÕâÀïµÄÅäÖÃÐÅÏ¢ÎÒÃÇÔÚuboot´úÂëÖÐÊ¹ÓÃµ½ÄÚ´æÊ±¾Í¿ÉÒÔ´ÓÕâÀïÌáÈ¡Ê¹ÓÃ(ÏëÏóubootÖÐÊ¹ÓÃ
// µ½ÄÚ´æµÄµØ·½¶¼²»ÊÇÖ±½ÓÓÃµØÖ·Êý×ÖµÄ£¬¶¼ÊÇÓÃºê¶¨ÒåµÄ)
#define CONFIG_NR_DRAM_BANKS    2          	/* we have 2 bank of DRAM Õâ¸öºêÖ¸µÄÊÇÎÒÃÇ¿ª·¢°åÒ»¹²ÓÐ¼¸Æ¬ÄÚ´æ*/
#define SDRAM_BANK_SIZE         0x10000000    /* 256MB lqm£»Ã¿Ò»Æ¬ÄÚ´æµÄ´óÐ¡*/
//#define SDRAM_BANK_SIZE         0x20000000    /* 1GB lqm*/
#define PHYS_SDRAM_1            MEMORY_BASE_ADDRESS 	/* SDRAM Bank #1 SDRAM1»ùµØÖ·*/
#define PHYS_SDRAM_1_SIZE       SDRAM_BANK_SIZE
#define PHYS_SDRAM_2            MEMORY_BASE_ADDRESS2 	/* SDRAM Bank #2 SDRAM1»ùµØÖ·*/
...
#define CFG_PHY_UBOOT_BASE	MEMORY_BASE_ADDRESS + 0x3e00000		// 33e00000  Õû¸öubootµÄÆðÊ¼µØÖ·
#define CFG_PHY_KERNEL_BASE	MEMORY_BASE_ADDRESS + 0x8000		// 33e08000
...
/* Fastboot variables */
// CFG_FASTBOOT_TRANSFER_BUFFERÅäÖÃfastboot¹¤×÷Ê±µÄ»º³åÇøµØÖ·£¬fastbootÔÚÖ´ÐÐÄ³Ð©¹¦ÄÜÊ±ÐèÒª´ó¿éÄÚ´æ
// ×ö»º³åÇø£¬ÕâÀï¾ÍÊÇÔÚ¸øËüÅäÖÃ»º³åÇø¡£
#define CFG_FASTBOOT_TRANSFER_BUFFER		(0x3E000000)//(0x40000000)
// CFG_FASTBOOT_TRANSFER_BUFFER_SIZE£¬fastboot»º³åÇøµÄ´óÐ¡
// ×¢Òâ: ºÜ¶àÍ¬Ñ§ÔÚÖ®Ç°Ë¢»úÊ±£¬ÉÕÂ¼ubootºÍzImage¶¼Õý³££¬µ«ÊÇÉÕÂ¼Android¾µÏñx210.imgÊ±´íÎó£¬ÌáÊ¾:
// image too large...(ÒâË¼ÊÇÎÄ¼þÌ«´ó)¡£Õâ¸ö´íÎóµÄÔ­Òò¾ÍÊÇx210.imgÌ«´óÁË£¬³¬³öÁËCFG_FASTBOOT_TRANSFER_BUFFER_SIZE
// ËùÒÔ·Å²»ÏÂÁË£¬ËùÒÔubootµÄfastbootÕâÀï±¨´íÁË¡£

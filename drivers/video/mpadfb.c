...
// Õâ¸öº¯Êý¶¼ÊÇ¸øframebufferÏà¹ØµÄÊý¾Ý½á¹¹½øÐÐ¸³Öµ
void fb_init()
{
	get_HCLKD();
	// infoÊÇÒ»¸ö½á¹¹Ìå£¬Õâ¸ö½á¹¹ÌåÃèÊöframebufferÇý¶¯ÖÐµÄÓ²¼þÉè±¸²ÎÊýµÄ
	// lcdÊÇ×Ô¼º¶¨ÒåµÄÊý¾Ý½á¹¹£¬ÀïÃæµÄÖµ¶¼ÊÇÈËÎªÅäÖÃ¸øÇý¶¯ÌåÏµµÄ
...
// LCD¸É»îµÄÄÚÈÝ¶¼ÔÚÕâÀïÁË
void mpadfb_init()
{
	// framebuffer³õÊ¼»¯ËùÐèÒªµÄ¶«Î÷£¬¶¼ÊÇ½á¹¹Ìå
	fb_init();			//lqm masked for test
	// ¿´Ãû×ÖÊÇLCD¶Ë¿ÚµÄ³õÊ¼»¯£¬¶Ë¿Ú¾ÍÊÇGPIO£¬Õâ¸öº¯ÊýÔÚ³õÊ¼»¯LCDÏà¹ØµÄSoCµÄÒý½Å
	lcd_port_init();
	// ¿´Ãû×ÖÊÇLCDµÄ¼Ä´æÆ÷µÄ³õÊ¼»¯£¬¾ÍÊÇSocµÄLCD¿ØÖÆÆ÷ÖÐµÄÄÇÐ©¼Ä´æÆ÷µÄ³õÊ¼»¯
	lcd_reg_init();
#ifdef CONFIG_CHECK_X210CV3
	init_logo();
#endif
// ÒÔÉÏÈý¸öº¯Êýµ÷ÓÃÍêºó£¬LCD³õÊ¼»¯¾Í½áÊøÁË£¬È»ºóÏòLCDÐ´¶«Î÷¾ÍÄÜÏÔÊ¾ÁË¡£µ«ÊÇÄãÒª×¢Òâ±³¹âÊÇ·ñ±»µãÁÁÁË¡£
// ±³¹â²»ÁÁ£¬Ö»ÊÇÄã¿´²»µ½LCDÉÏÓÐ¶«Î÷£¬²»ÄÜËµÃ÷LCD²»ÏÔÊ¾¡£ºóÃæÊ£ÏÂµÄÊÂÇé¾ÍÊÇÐ´¶«Î÷ºÍ¿ª±³¹âÁ½¸öÁË¡£
// Õâ¸öº¯Êý¸ºÔð°ÑlogoÍ¼Æ¬Ð´µ½LCDµÄfbÖÐÈ¥¡£

...
// 函数其实就是通过mmc设备编号来在系统中查找对应的mmc设备，mmc设备就是s3c_hsmmc_initialize函数中注册的
...
{
	return mmc_bwrite(0, start, blkcnt, src);		// mmc_bwrite函数里面就是和驱动有关了
}
...
int mmc_register(struct mmc *mmc)
{
	...
	list_add_tail(&mmc->link, &mmc_devices);		// list_add_tail是内核链表从尾部插入
...
int mmc_init(struct mmc *host)
{
	...
	/* Reset the Card */
	err = mmc_go_idle(host);		// 函数是让MMC进入idle状态，内部是向SD卡发送命令

	if (err)
		return err;

	/* Test for SD version 2 */
	err = mmc_send_if_cond(host);	// 函数内部是向SD卡发送命令
...
int board_mmc_init(bd_t *bis) __attribute__((weak, alias("__def_mmc_init")));	// 用__attribute__把__def_mmc_init取个别名board_mmc_init
...
// uboot中对硬件的操作，譬如网卡，SD卡等等，都是借用的Linux内核中的驱动来实现的，
// uboot的根目录底下有个drivers文件夹，这里放的全都是从Linux内核中移植过来的各种驱动源文件。
// mmc_initialize是具体硬件架构无关的一个MMC初始化函数，所有的使用了这套架构的代码都调用
// 这个函数来完成MMC的初始化。mmc_initialize中再调用board_mmc_init和cpu_mmc_init来完成具体
// 的硬件的MMC控制器初始化工作。
int mmc_initialize(bd_t *bis)
{
...
#else
	mmc = find_mmc_device(0);	// 0表示内部iNand；1表示SD卡
#endif
	if (mmc) {
		err = mmc_init(mmc);
		if (err)
	...
	// capacity是以扇区为单位的，(1<<9)表示512字节，而一个扇区有512字节
